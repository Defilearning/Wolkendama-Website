...
function ShopInfo({ shopData }) {
  return (
    <div className="flex px-5 py-4 max-h-[22rem]">
      <img
      ...
      <div className="flex flex-col flex-wrap gap-2 w-1/3">
        ...
        <div>
          ...
          <div className="flex gap-4">
            {shopData.itemFilter?.map((el) => (
              <p className="px-2 border border-slate-700 rounded-md text-slate-600">
                {el}
              </p>
            ))}
          </div>
        ...
        <div>
          ...
          {Object.keys(shopData.variant)?.map((el) => (
            <div className="flex">
        ...
        <div>
          ...
          {shopData.reserveItem?.length > 1 ? (
            Object.keys(shopData.reserveItem)?.map((el, i) => (
              <div className="flex">
